...
export default function App() {
  ...
  // Forma d'onda animata — 5 barre con timing irregolare per sembrare reattiva all'audio
  const micWaveform = (color: string) => (
    <span aria-hidden="true" style={{ display: 'inline-flex', alignItems: 'center', gap: '2.5px', height: '22px', marginLeft: '4px', marginRight: '2px', flexShrink: 0 }}>
      {([
        ['waveBar1', '0.57s', '16px'],
        ['waveBar2', '0.78s', '20px'],
        ['waveBar3', '0.51s', '22px'],
        ['waveBar4', '0.69s', '18px'],
        ['waveBar5', '0.88s', '14px'],
      ] as const).map(([anim, dur, h], i) => (
        <span key={i} style={{ display: 'block', width: '3px', height: h, background: color, borderRadius: '99px', transformOrigin: 'center', animation: `${anim} ${dur} ease-in-out infinite` }} />
      ))}
    </span>
  );
      ...
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
                    ...
                    {([
                      {
                        ...
                        items: [
                          'Scrivi una frase in italiano nel campo di testo e premi "Traduci" per la traduzione rapida',
                          'Premi "Spiega con AI" per una traduzione contestualizzata con pronuncia fonetica e spiegazione grammaticale',
                          'Usa "DETTA" per dettare con la voce invece di scrivere: quando il microfono è attivo vedi la forma d\'onda animata nel pulsante',
                          'Scegli la lingua di destinazione tra le bandierine (29+ lingue disponibili)',
                        ],
                      },
                      {
                        icon: '🔬', title: 'X-Ray grammaticale',
                        items: [
                          'Dopo la traduzione, tocca qualsiasi parola per analizzarla con l\'AI',
                          'Vedrai: parte del discorso, genere/numero, tempo verbale, radice etimologica e curiosità',
                          'Ottimo per capire come funziona la grammatica, non solo cosa significa una parola',
                        ],
                      },
                      {
                        icon: '🔊', title: 'Pronuncia & IPA',
                        items: [
                          'Tocca 🔊 per ascoltare la traduzione con sintesi vocale nativa',
                          'Regola la velocità con lo slider (🐢 lento — 🚀 veloce) e scegli quante ripetizioni',
                          'Tocca "IPA + sillabazione" per vedere la trascrizione fonetica internazionale e la suddivisione in sillabe della parola tradotta',
                          'Scegli la voce preferita dal selettore voci (dipende dal dispositivo)',
                        ],
                      },
                      {
                        icon: '🎤', title: 'Dettatura vocale',
                        items: [
                          'Premi "DETTA" per attivare il microfono: parla in italiano e il testo appare automaticamente',
                          'Mentre parli, nel pulsante compare una forma d\'onda animata a 5 barre — feedback visivo immediato',
                          'Supportato su Chrome, Edge e Safari; su Firefox non disponibile',
                          'Utile su smartphone per tradurre senza digitare',
                        ],
                      },
                      {
                        icon: '🎙️', title: 'Pratica pronuncia',
                        items: [
                          'Dopo aver ottenuto una traduzione, premi "PRATICA PRONUNCIA"',
                          'Pronuncia la frase in lingua straniera — la forma d\'onda mostra che stai registrando',
                          'Il riconoscimento vocale confronta quello che hai detto con la traduzione corretta',
                          'Ricevi un punteggio e vedi quali parole hai pronunciato bene o no',
                        ],
                      },
                      {
                        icon: '🔁', title: 'Shadowing',
                        items: [
                          'Apri la sezione "Shadowing" e premi "Genera nuova frase" — l\'AI crea una frase nella lingua scelta',
                          'Ascoltala con "Riascolta", poi premi "Ripeti ora!" e parla: la forma d\'onda conferma che stai registrando',
                          'Ricevi uno score che misura quanto la tua pronuncia corrisponde all\'originale',
                          'Tecnica usata dai poliglotti: ascolta e ripeti immediatamente per assorbire ritmo e intonazione',
                        ],
                      },
                      {
                      ...
                      {
                        icon: '⭐', title: 'Segnalibri & Quiz Tatoeba',
                        items: [
                          'Salva una traduzione con ⭐ per aggiungerla al tuo vocabolario personale',
                          'Nella sezione "Vocabolario" rivedi tutte le parole salvate; tocca "Quiz" per testarti con 4 opzioni a risposta multipla',
                          'Il "Quiz Tatoeba" usa frasi reali scritte da madrelingua: indovina la traduzione mancante tra 4 opzioni',
                          'Tatoeba funziona anche offline se hai già caricato l\'app almeno una volta',
                        ],
                      },
                      {
                        icon: '👤', title: 'Profilo personale',
                        items: [
                          'Inserisci nome, età, livello di lingua, occupazione, città e interessi',
                          'L\'AI usa queste informazioni per personalizzare esempi e spiegazioni',
                          'Es: se sei medico, gli esempi saranno in ambito medico; se sei principiante, le spiegazioni saranno più semplici',
                        ],
                      },
                      {
                        icon: '♿', title: 'Accessibilità',
                        items: [
                          '"Modalità ipovedenti": ingrandisce testi, aumenta contrasti e semplifica il layout per chi ha difficoltà visive',
                          '"TalkBack in-app": aggiunge descrizioni testuali vocali pensate per chi usa screen reader (Android TalkBack / iOS VoiceOver)',
                          'Tutte le icone decorative nei pulsanti sono nascoste agli screen reader — vengono letti solo i testi e le etichette',
                          '"Modalità accessibile" riduce le animazioni e aumenta la leggibilità generale',
                        ],
                      },
                      {
                        icon: '📵', title: 'Offline & Aggiornamenti',
                        items: [
                          'Installa l\'app dal browser (pulsante "Installa") per usarla come app nativa senza browser',
                          'Dopo il primo caricamento l\'app funziona completamente offline: traduzioni in cache, font, quiz e pronuncia',
                          'Quando esce una nuova versione, compare un banner viola "Nuova versione disponibile" — tocca "Aggiorna" per installarla',
                          'Dopo l\'aggiornamento compare un toast verde in basso con la conferma e il numero di versione installata',
                        ],
                      },
                    ]).filter(({ title, items }) => {
        ...
        <section className="input-section" style={styles.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '16px' }}>
            ...
            >
              ...
              {isListening ? micWaveform('rgba(255,255,255,0.92)') : null}
              {isListening ? 'FERMA' : 'DETTA'}
            </button>
            {dictError && (
        ...
        {translatedText && (
          <section aria-live="polite" aria-label="Risultato traduzione" style={{ ...styles.card, border: '2px solid #10b981' }}>
            ...
            >
              <Mic aria-hidden="true" size={18} />
              {isPracticing ? micWaveform('rgba(30,24,10,0.85)') : null}
                ...
                <div style={{ backgroundColor: '#0f172a', borderRadius: '8px', padding: '12px', border: '1px solid #6d28d9' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    >
                      ...
                      {shadowStep === 'listening' ? micWaveform('rgba(30,24,10,0.85)') : null}
