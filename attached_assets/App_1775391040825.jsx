...
import { 
  Mic, Volume2, Send, Loader2, AlertCircle, CloudLightning 
} from 'lucide-react';

// --- CONFIGURAZIONE GOOGLE AI ---
import { GoogleGenerativeAI } from "@google/generative-ai";

// ⚠️ METTI QUI LA TUA CHIAVE
const API_KEY = "AIzaSyAAks5cBFb8HGoq7GooYE5qAAK8n7xCjX0"; 
const genAI = new GoogleGenerativeAI(API_KEY);

// Inizializzazione pulita (senza systemInstruction che causa l'errore 400)
...
export default function App() {
  ...
  const handleTranslate = async () => {
    ...
    try {
      // Prompt "tutto in uno" per evitare errori di compatibilità JSON
  ...
  const startPracticeSession = () => {
    ...
    recognition.onstart = () => { setIsPracticing(true); setPracticeFeedback('Dì la frase tradotta...'); };
  ...
  const styles = {
    main: { minHeight: '100vh', backgroundColor: '#0f172a', color: '#f8fafc', padding: '20px', fontFamily: 'sans-serif' },
    card: { backgroundColor: '#1e293b', borderRadius: '15px', padding: '20px', marginBottom: '20px', border: '1px solid #334155' },
    btn: { width: '100%', padding: '12px', borderRadius: '10px', border: 'none', backgroundColor: '#3b82f6', color: '#fff', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }
  };
      ...
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
        ...
        <section style={styles.card}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
            {LANGUAGES.map(l => (
              <button key={l.code} onClick={() => setSelectedLang(l.code)} style={{ padding: '8px', backgroundColor: selectedLang === l.code ? '#3b82f6' : '#334155', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
        ...
        <section style={styles.card}>
          <textarea 
            style={{ width: '100%', height: '80px', backgroundColor: '#0f172a', color: '#fff', border: '1px solid #334155', padding: '10px', borderRadius: '8px', resize: 'none' }} 
            value={inputText} 
            onChange={(e) => setInputText(e.target.value)} 
            placeholder="Scrivi in italiano..." 
          />
          <button style={{ ...styles.btn, backgroundColor: isListening ? '#ef4444' : '#334155' }} onClick={startInputSpeech}>
             <Mic size={18} /> DETTA
          </button>
          <button style={{ ...styles.btn, backgroundColor: '#6366f1' }} onClick={handleTranslate} disabled={loading}>
            {loading ? <Loader2 className="animate-spin" size={18} /> : <Send size={18} />} TRADUCI
          </button>
          {error && <p style={{color: '#ef4444', fontSize: '0.8rem', marginTop: '10px'}}><AlertCircle size={14} /> {error}</p>}
        </section>
        ...
        {translatedText && (
          <section style={{ ...styles.card, borderLeft: '4px solid #10b981' }}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
               ...
               <Volume2 size={24} color="#10b981" style={{cursor: 'pointer'}} onClick={() => {
            ...
            <button style={{ ...styles.btn, backgroundColor: isPracticing ? '#f59e0b' : '#10b981' }} onClick={startPracticeSession}>
